...
app.get('/mentions/new', (req, res, next) => {
   const news = db.tweets.root.get('new').value();
   res.json(news);
});

app.get('/mentions/old', (req, res, next) => {
   const old = db.tweets.root.get('old').value();
   res.json(old);
});

app.get('/mentions/ids', (req, res, next) => {
   const max_id = db.tweets.root.get('max_id').value();
   const max_id_overall = db.tweets.root.get('max_id_overall').value();
   const since_id = db.tweets.root.get('since_id').value();
   res.json({ max_id, max_id_overall, since_id });
});

app.get('/mentions/tweeted', (req, res, next) => {
   const tweeted = db.tweets.root.get('tweeted').value();
   res.json(tweeted);
});

app.get('/mentions/texts', (req, res, next) => {
   const texts = db.tweets.root.get('texts').value();
   res.json(texts);
});
...
app.post('/mentions/max_id', (req, res) => {
   ...
   db.tweets.root.update('max_id', max_id => req.body.max_id).write();
   res.json(req.max_id);
});
...
app.post('/mentions/since_id', (req, res) => {
   ...
   db.tweets.root.update('since_id', since_id => req.body.since_id).write();
